...
IODA_DL ioda_group* ioda_Engines_HH_createMemoryFile(
  ...
  C_TRY;
  res = new ioda_group;
  res->g = ioda::Engines::HH::createMemoryFile(std::string{filename},
                                               ioda::Engines::BackendCreateModes::Truncate_If_Exists,
                                               flush_on_close, (size_t)increment_len_bytes);
  C_CATCH_RETURN_FREE(res, NULL, res);
}

IODA_DL ioda_group* ioda_Engines_HH_openFile(const char* filename, ioda_Engines_BackendOpenModes mode) {
  ioda_group* res = nullptr;
  C_TRY;
  res = new ioda_group;
  ...
  res->g = ioda::Engines::HH::openFile(std::string{filename}, m.at(mode));
  C_CATCH_RETURN_FREE(res, NULL, res);
}

IODA_DL ioda_group* ioda_Engines_HH_createFile(const char* filename, ioda_Engines_BackendCreateModes mode) {
  ioda_group* res = nullptr;
  C_TRY;
  res = new ioda_group;
  ...
  const std::map<ioda_Engines_BackendCreateModes, ioda::Engines::BackendCreateModes> m{
    {ioda_Engines_BackendCreateModes_Truncate_If_Exists,
     ioda::Engines::BackendCreateModes::Truncate_If_Exists},
    {ioda_Engines_BackendCreateModes_Fail_If_Exists, ioda::Engines::BackendCreateModes::Fail_If_Exists}};
  if (!m.count(mode)) throw;  // jedi_Unimplemented;

  res->g = ioda::Engines::HH::createFile(std::string{filename}, m.at(mode));
  C_CATCH_RETURN_FREE(res, NULL, res);
}

IODA_DL ioda_group* ioda_Engines_constructFromCmdLine(int argc, char** argv, const char* defaultFilename) {
  ioda_group* res = nullptr;
  C_TRY;
  res = new ioda_group;
