...
int ioda_has_variables_exists(const ioda_has_variables* vars, size_t sz, const char* name) {
  ...
  bool exists = vars->vars.exists(std::string(name, sz));
  C_CATCH_AND_RETURN((exists) ? 1 : 0, -1);
}

bool ioda_has_variables_remove(ioda_has_variables* vars, size_t sz, const char* name) {
  C_TRY;
  Expects(vars != nullptr);
  Expects(name != nullptr);
  vars->vars.remove(std::string(name, sz));
  C_CATCH_AND_RETURN(true, false);
}

ioda_variable* ioda_has_variables_open(const ioda_has_variables* vars, size_t sz,
                                       const char* name) {
  ...
  Expects(res != nullptr);
  res->var = vars->vars.open(std::string(name, sz));
  C_CATCH_RETURN_FREE(res, nullptr, res);
}
