...
std::shared_ptr<Attribute> Attribute::write(gsl::span<const char> data,
  ...
  if (dtype == *dtype_) {
    ...
    const auto internal_type = dtype_->getHDF5Type();
    const auto from_type = dtype.getHDF5Type();

    // Determine the necessary size for the converted data buffer. HDF5 uses
    // an in-place conversion strategy.
    const size_t nelements = m_select.npoints();
    const size_t min_sz_type = std::max(internal_type.getSize(), from_type.getSize());
    const size_t buf_sz = nelements * min_sz_type;
...
std::shared_ptr<Attribute> Attribute::read(gsl::span<char> data, const Type & dtype) {
  ...
  if (dtype == *dtype_) {
    ...
    // Then, convert the data to the requested type and return.
    const auto internal_type = dtype_->getHDF5Type();
    const auto to_type = dtype.getHDF5Type();

    // Determine the necessary size for the converted data buffer. HDF5 uses
    // an in-place conversion strategy.
    const size_t nelements = m_select.npoints();
    const size_t min_sz_type = std::max(internal_type.getSize(), to_type.getSize());
    const size_t buf_sz = nelements * min_sz_type;
