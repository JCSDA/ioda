...
void setupColumnInfo(const ContainerFacade &container,
  ...
  if (!ignoreChannels && !container.channelNumbers().empty()) {
    ColumnInfo col;
...
void setODBColumn(std::map<std::string, std::string> &columnMappings, const ColumnInfo v,
  ...
  // TODO(lewisn-met): this case should include char (bools stored as int)
  // Will require ioda-data and KGO update to fix.
...
void setODBBodyColumn(const ColumnInfo &v, odc::Writer<>::iterator writer, int &column_number) {
  ...
  if (v.column_type == ContainerVariableType::Int || v.column_type == ContainerVariableType::Int64 || v.column_type == ContainerVariableType::Char) {
...
void fillFloatArray(const ContainerFacade &container, const std::string varname, 
  ...
  if (container.hasVariable(varname)) {
    ...
    throw eckit::BadParameter("Variable specified in call does not exist: " + varname, Here());
  }
}
...
void readBodyColumns(const ContainerFacade &container, const ColumnInfo &column, const std::string v,
  ...
  if (column.column_type == ContainerVariableType::Int || column.column_type == ContainerVariableType::Int64) {
    fillIntArray(container, obsspacename, column.column_type, data_store_tmp);
    ...
    fillFloatArray(container, obsspacename, data_store_tmp, odb_type, extendeds);
  } else if (column.column_type == ContainerVariableType::String) {
    std::vector<std::string> buf = container.variableValues<std::string>(obsspacename);
    int num_cols = 1 + ((column.string_length - 1) / 8);
    for (int c = 0; c < num_cols; c++) {
...
void createFile(const ODC_Parameters &odcparams, ContainerFacade &container) {
  ...
  std::vector<int> channel_numbers = container.channelNumbers();

  if (!channel_numbers.empty() && !odcparams.ignoreChannelDimensionWrite) {
    number_of_rows *= channel_numbers.size();
    number_of_channels = channel_numbers.size();
  }
  if (container.hasVariable("MetaData/extendedObsSpace")) {
    extendeds = container.variableValues<int32_t>("Metadata/extendedObsSpace");
  } else {
  ...
  for (const auto &v : column_infos) {
    readColumn(container, v, data_store, number_of_locations, number_of_channels,
