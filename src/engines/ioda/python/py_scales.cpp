...
void setupNewDimensionScales(pybind11::module& m) {
  ...
  nds.def(py::init<const std::string, const std::type_index&, Dimensions_t, Dimensions_t, Dimensions_t>())
...
#define InstTmpl(typen, an, cn, typ)                                                                    \
  {                                                                                                     \
    py::class_<NewDimensionScale<typ>, NewDimensionScale_Base, std::shared_ptr<NewDimensionScale<typ>>> \
      newnds(mNDS, typen);                                                                              \
    newnds.doc() = "New dimension scale of type " typen;                                                \
    newnds                                                                                              \
      .def(py::init<const std::string, Dimensions_t, Dimensions_t, Dimensions_t>(), py::arg("name"),    \
           py::arg("size"), py::arg("maxSize"), py::arg("chunkingSize"))                                \
