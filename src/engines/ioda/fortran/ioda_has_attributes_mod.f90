...
contains
   ...
   subroutine ioda_has_attributes_list(this, vstr)
      implicit none
      class(ioda_has_attributes) :: this
      class(ioda_vecstring) :: vstr
      if (.not. c_associated(vstr%data_ptr)) then
!!!  prevent memory leak
   ...
   subroutine ioda_has_attributes_open(this, n, fstr, att)
      implicit none
      class(ioda_has_attributes) :: this
      class(ioda_attribute), intent(out) :: att
      integer(int64) :: n
      character(len=*), intent(in) :: fstr
      logical :: r
      type(c_ptr) :: cstr

      if (.not. c_associated(att%data_ptr)) then
!!! prevent memory leak
         call ioda_attribute_dtor(att)
      end if
      cstr = ioda_f_string_to_c_dup(fstr)
