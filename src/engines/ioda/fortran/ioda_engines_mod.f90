...
module ioda_engines_mod
   use, intrinsic :: iso_c_binding, only: &
       c_associated, c_int32_t, c_int64_t, c_ptr
   use, intrinsic :: iso_fortran_env, only: int32, int64
   use :: ioda_group_mod, only: ioda_group
   use :: cxx_vector_string_mod, only: &
       cxx_vector_string, cxx_vector_string_c_alloc, cxx_vector_string_init
   ...
   interface
      ...
      & bind(C, name="ioda_engines_c_obstore_create_root_group")
         ...
         implicit none
         type(c_ptr) :: new_grp
      end function ioda_engines_c_obstore_create_root_group
      ...
      end function ioda_engines_c_hh_create_memory_file
      ...
      & bind(C, name="ioda_engines_c_hh_open_memory_file")
         ...
         type(c_ptr), value :: name
         integer(c_int64_t), value :: increment_len
      end function ioda_engines_c_hh_open_memory_file
      ...
      end function ioda_engines_c_hh_open_file
      ...
      & bind(C, name="ioda_engines_c_hh_create_file")
         ...
         type(c_ptr), value :: name
         integer(c_int32_t), value :: backend_mode
      end function ioda_engines_c_hh_create_file
      ...
      end function ioda_engines_c_construct_from_command_line
   end interface
contains
   ...
   end function ioda_engines_obstore_create_root_group
   ...
   function ioda_engines_hh_create_memory_file(fname, increment_len) result(new_grp)
      ...
      call c_free(fname_p)
   end function ioda_engines_hh_create_memory_file
   ...
   function ioda_engines_hh_open_memory_file(fname, increment_len) result(new_grp)
      ...
      call c_free(fname_p)
   end function ioda_engines_hh_open_memory_file

   function ioda_engines_hh_open_file(fname, open_mode) result(new_grp)
      implicit none
      type(ioda_group) :: new_grp
      character(len=*), intent(in) :: fname
      integer(int32), intent(in) :: open_mode
      integer(int32) :: backend_mode
      type(C_ptr) :: fname_p

      select case (open_mode)
      case (0:3)
          backend_mode = open_mode
      case default
          backend_mode = 0
      end select
      fname_p = f_string_to_c_dup(fname)
      new_grp%data_ptr = ioda_engines_c_hh_open_file(fname_p, backend_mode)
      call c_free(fname_p)
   end function ioda_engines_hh_open_file
   ...
   function ioda_engines_hh_create_file(fname, create_mode) result(new_grp)
      ...
      call c_free(fname_p)
   end function ioda_engines_hh_create_file
   ...
   end subroutine ioda_command_line_to_vecstring
   ...
   end function ioda_engines_construct_from_command_line
end module ioda_engines_mod
