...
int main(int argc, char** argv) {
  ...
  struct ioda_variable_creation_parameters *params_default = NULL, *params_dimchannel = NULL, *p1 = NULL;
  ...
  dim_location = ioda.Has_Variables.create_int(gvars, "Location", 1, &num_locs, &num_locs, params_default);
  if (!dim_location) doErr;
  if (!ioda.Variable.setIsDimensionScale(dim_location, "Location")) doErr;
  ...
  if (!ioda.VariableCreationParams.setIsDimensionScale(params_dimchannel, "ATMS Channel Number")) doErr;
  if (ioda.VariableCreationParams.isDimensionScale(params_dimchannel) <= 0) doErr;

  dim_channel =
    ioda.Has_Variables.create_int(gvars, "ATMS Channel", 1, &num_channels, &num_channels, params_dimchannel);
  ...
  // C++ line 98
  var_longitude =
    ioda.Has_Variables.create_float(gvars, "Longitude", 1, &num_locs, &num_locs, params_default);
  ...
  v_lon_valid_range = ioda.Has_Attributes.create_float(var_lon_atts, "valid_range", 1, valid_range_dims);
  if (!v_lon_valid_range) doErr;
  if (!ioda.Attribute.write_float(v_lon_valid_range, 2, lon_valid_range)) doErr;

  v_lon_units = ioda.Has_Attributes.create_str(var_lon_atts, "units", 1, str_dims);
  if (!v_lon_units) doErr;
  if (!ioda.Attribute.write_str(v_lon_units, 1, &lon_units)) doErr;

  v_lon_long_name = ioda.Has_Attributes.create_str(var_lon_atts, "long_name", 1, str_dims);
  if (!v_lon_long_name) doErr;
  if (!ioda.Attribute.write_str(v_lon_long_name, 1, &lon_long_name)) doErr;

  // C++ line 110
  var_latitude = ioda.Has_Variables.create_float(gvars, "Latitude", 1, &num_locs, &num_locs, params_default);
  ...
  v_lat_valid_range = ioda.Has_Attributes.create_float(var_lat_atts, "valid_range", 1, valid_range_dims);
  if (!v_lat_valid_range) doErr;
  if (!ioda.Attribute.write_float(v_lat_valid_range, 2, lat_valid_range)) doErr;

  v_lat_units = ioda.Has_Attributes.create_str(var_lat_atts, "units", 1, str_dims);
  if (!v_lat_units) doErr;
  if (!ioda.Attribute.write_str(v_lat_units, 1, &lat_units)) doErr;

  v_lat_long_name = ioda.Has_Attributes.create_str(var_lat_atts, "long_name", 1, str_dims);
  ...
  var_tb = ioda.Has_Variables.create_float(gvars, "Brightness Temperature", 2, sz_tb, sz_tb, params_default);
  if (!var_tb) doErr;
  ...
  v_tb_valid_range = ioda.Has_Attributes.create_float(var_tb_atts, "valid_range", 1, valid_range_dims);
  if (!v_tb_valid_range) doErr;
  if (!ioda.Attribute.write_float(v_tb_valid_range, 2, tb_valid_range)) doErr;

  v_tb_units = ioda.Has_Attributes.create_str(var_tb_atts, "units", 1, str_dims);
  if (!v_tb_units) doErr;
  if (!ioda.Attribute.write_str(v_tb_units, 1, &tb_units)) doErr;

  v_tb_long_name = ioda.Has_Attributes.create_str(var_tb_atts, "long_name", 1, str_dims);
  ...
  // C++ line 166
  var_sza = ioda.Has_Variables.create_float(gvars, "Solar Zenith Angle", 1, &num_locs, &num_locs, p1);
  if (!var_sza) doErr;
  if (ioda.Variable.isDimensionScaleAttached(var_sza, 0, dim_location) <= 0) doErr;
  if (!ioda.Variable.detachDimensionScale(var_sza, 0, dim_location)) doErr;  // Just trying to check...
  if (ioda.Variable.isDimensionScaleAttached(var_sza, 0, dim_location) != 0) doErr;
  ...
  v_sza_valid_range = ioda.Has_Attributes.create_float(var_sza_atts, "valid_range", 1, valid_range_dims);
  if (!v_sza_valid_range) doErr;
  if (!ioda.Attribute.write_float(v_sza_valid_range, 2, sza_valid_range)) doErr;

  v_sza_units = ioda.Has_Attributes.create_str(var_sza_atts, "units", 1, str_dims);
