...
program variables_example
    use, intrinsic :: iso_fortran_env, only: error_unit, int32, int64, real64

    use :: ioda_variable_mod, only: ioda_variable, ioda_variable_init
    use :: ioda_has_variables_mod, only: ioda_has_variables, ioda_has_variables_init
    use :: cxx_vector_string_mod, only: cxx_vector_string, cxx_vector_string_init
    use :: ioda_group_mod, only: ioda_group
    use :: ioda_engines_mod, only: ioda_engines_construct_from_command_line
...
!        call ioda_has_variables_init(gvars)
    ...
    if (dims%get_dimensionality() /= 3) then
