...
int main(int argc, char** argv) {
  ...
  charatt1                  = ioda.Group.atts.create_char(gatts, 6, "char-1", 1, char1_dims);
  if (!charatt1) doErr;
  ...
  f1 = ioda.Group.atts.open(gatts, 7, "float-1");
  if (!f1) doErr;
  d1 = ioda.Group.atts.open(gatts, 8, "double-1");
  if (!d1) doErr;
  ...
  if (ioda.Has_Attributes.exists(gatts, 9, "int-att-1") <= 0) doErr;

  if (!ioda.Has_Attributes.rename_att(gatts, 9, "int-att-2", 10, "int-add-2b")) doErr;

  ioda.Attribute.destruct(intatt3);
  intatt3 = NULL;

  if (!ioda.Has_Attributes.remove(gatts, 9, "int-att-3")) doErr;
